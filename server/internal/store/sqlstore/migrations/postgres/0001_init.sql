-- 0001_init (Postgres dialect): core catalog, users, progress, lists, and jobs.
-- Mirrors migrations/sqlite/0001_init.sql. Dialect notes: millisecond timestamps and
-- file sizes are BIGINT (PG INTEGER is 32-bit); SQLite REAL becomes DOUBLE PRECISION;
-- "user" is quoted (reserved word); 0/1 integer booleans are kept as INTEGER so the
-- shared repository SQL behaves identically on both engines.
