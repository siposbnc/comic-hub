...
type collectionRepo struct{ db *DB }
...
func (r *collectionRepo) AddItems(ctx context.Context, collectionID string, bookIDs []string) error {
	...
	for _, bookID := range bookIDs {
		pos += positionGap
		// Skip books already in the collection (keep their place) via ON CONFLICT DO NOTHING.
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO collection_item (collection_id, book_id, position) VALUES (?, ?, ?) ON CONFLICT DO NOTHING`,
