...
func TestCB7Extensions(t *testing.T) {
	got := CB7{}.Extensions()
	want := map[string]bool{"cb7": true, "7z": true}
	if len(got) != len(want) {
...
// TestCB7Decode exercises the real 7-Zip decode path against a committed fixture
// (testdata/sample.cb7: pages p1/p2/p10 + ComicInfo.xml). It mirrors the CBZ/CBR tests so
// every archive format is held to the same behavior.
func TestCB7Decode(t *testing.T) {
	src, err := CB7{}.Open(filepath.Join("testdata", "sample.cb7"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	defer src.Close()
